...
flowchart TD
    ...
    D --> |"CandidateGroup[]"| E["Фаза 3: Разрешение"]

    E --> |"Online: API"| F1["iNaturalist API + кэш"]
    E --> |"Offline: SQL"| F2["noo-garden PostgreSQL"]
    F1 --> G["ResolvedCandidate[]"]
    F2 --> G

    G --> H{"Фаза 4: identified?"}
    H --> |"Нет → LLM-обогатитель"| I["Альтернативные названия → повторный поиск"]
    I --> G
...
| Значение | Описание |
|----------|----------|
| `"dictionary"` | Найден dictionary matching (поиск по известным названиям) |
| `"latin_regex"` | Найден regex-детектором латинских биномиалов |
| `"llm"` | Извлечён LLM-экстрактором |

В Python: `Literal["dictionary", "latin_regex", "llm"]`.
...
| `normalized` | `str` | Нормализованная форма: lowercase + замена ё→е (для кириллицы). Используется для группировки и поиска. |
| `lemmatized` | `str` | Лемматизированная форма (каждое слово лемматизировано, затем lowercase). Это **ключ группировки** на Фазе 2: кандидаты с одинаковым `lemmatized` объединяются. |
...
| `dictionary_taxon_ids` | `list[int]` | Список `taxon_id`, на которые маппится это название (из dictionary matching). Пуст для кандидатов из regex и LLM. Используется для расчёта confidence (фактор неоднозначности). |
...
Представляет один «уникальный кандидат», который будет разрешён через основной
источник данных ровно один раз. Содержит все вхождения этого кандидата в тексте.

| Поле | Тип | Описание |
|------|-----|----------|
| `normalized` | `str` | Нормализованная форма (из наилучшего кандидата группы). Используется как поисковый запрос. |
| `lemmatized` | `str` | Лемматизированная форма — ключ, по которому кандидаты были сгруппированы. |
| `method` | `ExtractionMethod` | Метод извлечения с наивысшим приоритетом в группе (dictionary > regex > LLM). Если один и тот же фрагмент найден несколькими методами — берётся dictionary. |
...
| `dictionary_taxon_ids` | `list[int]` | Объединение `dictionary_taxon_ids` из всех кандидатов группы. При группировке по лемме объединение выполняется только если множества пересекаются (или хотя бы одно пусто). Это предотвращает объединение омонимов, маппящихся на разные таксоны. |
...
таксономическим рангам. В online режиме заполняется из `ancestry` ответа iNaturalist API,
в offline режиме — из денормализованных полей таблицы `taxa` базы noo-garden.
...
Один результат поиска таксона через основной источник данных (iNaturalist API или noo-garden).
...
| `taxonomy` | `TaxonomyInfo` | Полная таксономическая иерархия. |
| `taxon_common_name_en` | `str \| None` | Предпочитаемое английское народное название. `None`, если недоступно. |
| `taxon_common_name_loc` | `str \| None` | Предпочитаемое народное название для locale из конфигурации (например, «Липа» для locale=ru). `None`, если источник данных не имеет common name для этого locale. |
...
| `taxon_url` | `str` | URL таксона на iNaturalist.org (например, `https://www.inaturalist.org/taxa/54586`). Формируется из `taxon_id`. |
| `taxon_names` | `list[str]` | Список всех альтернативных названий таксона. Используется внутренне в `DefaultIdentificationResolver` для улучшения точности сопоставления имён. Не сериализуется в JSON-вывод. |
| `score` | `float` | Релевантность. В online режиме — `score` из ответа iNaturalist API. В offline режиме — синтетический score на основе типа совпадения (exact/prefix/FTS) и флага `is_preferred`. Используется для сортировки matches. |

### LlmEnrichmentResponse

Ответ LLM-обогатителя (Фаза 4). Содержит альтернативные названия, предложенные LLM
для кандидата, не разрешённого через основной источник данных. Соответствует полю 
`llm_response` в JSON-выводе.
...
| `matches` | `list[TaxonMatch]` | Результаты из основного источника данных (до 5). Формат элементов совпадает с JSON-схемой вывода. |
...
| `"No matches found"` | Источник данных не нашёл ни одного таксона по запросу. |
